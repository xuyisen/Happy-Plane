...
bool ThirdScene::init()
{
	...
	word = FlyWord::create(6, Vec2(384, 512));
...
void ThirdScene::mangerMainBullet(float time) {
	...
	CallFuncN *fun = CallFuncN::create(CC_CALLBACK_1(ThirdScene::removeBullet, this));
...
void ThirdScene::MainMenuGame(Ref *object) {
	//跳转至主菜单页面
	Director *dir = Director::getInstance();
	dir->replaceScene(StartScene::createScene());
}

void ThirdScene::scoreShowControl() {
...
//分数
void ThirdScene::countScoreUse() {
	//ScaleTo * max = ScaleTo::create(0.25, 1.5);
	//ScaleTo * min = ScaleTo::create(0.25, 0.5);
	//ScaleTo * start = ScaleTo::create(0.25, 1);
	CallFunc* textShow = CallFunc::create(CC_CALLBACK_0(ThirdScene::scoreShowControl, this));
