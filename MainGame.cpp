...
bool MainGame::init()
{
	...
	{
		//定时随机产生敌机
		//schedule(schedule_selector(MainGame::managerEnemyPlanes), 1.0f);
		//定时随机产生Buff道具
	//	schedule(schedule_selector(MainGame::manageBuff), 1.0f);
	...
	/*碰撞检测*/
	{
		...
		m_event->onContactBegin = [this](PhysicsContact& contact)
		{
			...
			if (spriteA && spriteA->getTag() == 0
				&& spriteB && spriteB->getTag() <=3&& spriteB->getTag()>=1) {
			...
			else if (spriteA && spriteB &&spriteA->getTag() == 6
				&& spriteB->getTag() == 4) {
				heroPlaneG->IsAddBlood = true;
				healthPoint->setString("HP: 100");
				refreshTheHP();
			...
			else if (spriteA && spriteB &&spriteA->getTag() == 7
				&& spriteB->getTag() == 4) {
				heroPlaneG->IsThreeShooting = true;
				removeBuff(spriteA);
			}
		...
		m_event->onContactPreSolve = [](PhysicsContact& contact,
...
void MainGame::mangerMainBullet(float time) {
	...
	{
		if (heroPlaneG->IsDoubleShooting)
		{
			...
			blt2->type = bullet_main;
			blt2->setTag(0);
			heroPlaneG->IsRocketLoading = false;
			{
				//设置物理body
				auto body = PhysicsBody::createBox(blt1->getContentSize());
				//body->setContactTestBitmask(0x08);
...
void MainGame::changeTheScore() {
	//heroPlaneG = HeroPlane::createHeroPlane("heroPlane.png");
	/*
	auto nextScore = heroPlaneG->m_score + addtionalScore;
	heroPlaneG->m_score = heroPlaneG->m_score + addtionalScore;
	auto temp = heroPlaneG->m_score;
	log("HeroScore: %d", temp);
	*/
	...
	//_eventDispatcher->removeEventListenersForTarget(heroPlaneG);
...
void MainGame::refreshTheScore() {
	...
	/*CallFunc* HPtextShow = CallFunc::create(CC_CALLBACK_0(MainGame::changeTheScore, this));
	Sequence* list = Sequence::create(HPtextShow, NULL);
